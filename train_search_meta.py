...
DATASET_CLASSES1 = 47
# birds
DATASET_CLASSES2 = 200
# imagenet
DATASET_CLASSES3 = 200
...
def main():
    ...
    # dtd
    train_data1 = dtd(args, "train")
    val_data1 = dtd(args, "val")

    # birds
    train_data2 = birds(args, "train")
    val_data2 = birds(args, "val")

    # ImageNet
    train_data3 = imagenet(args, "train")
    val_data3 = imagenet(args, "val")

    input_channels = 3
    logging.info('------- Dataset: --------: %s %i', args.dataset, input_channels)

    # 1 dataset
    train_queue1 = torch.utils.data.DataLoader(
        train_data1, batch_size=args.batch_size, shuffle=True,
        pin_memory=True, num_workers=args.workers)

    valid_queue1 = torch.utils.data.DataLoader(
        val_data1, batch_size=args.batch_size, shuffle=False,
        pin_memory=True, num_workers=args.workers)

    # 2 dataset
    train_queue2 = torch.utils.data.DataLoader(
        train_data2, batch_size=args.batch_size, shuffle=True,
        pin_memory=True, num_workers=args.workers)

    valid_queue2 = torch.utils.data.DataLoader(
        val_data2, batch_size=args.batch_size, shuffle=False,
        pin_memory=True, num_workers=args.workers)

    # 3 dataset
    train_queue3 = torch.utils.data.DataLoader(
        train_data3, batch_size=args.batch_size, shuffle=True,
        pin_memory=True, num_workers=args.workers)

    valid_queue3 = torch.utils.data.DataLoader(
        val_data3, batch_size=args.batch_size, shuffle=False,
        pin_memory=True, num_workers=args.workers)
    ...
    for sp in range(len(num_to_keep)):
        model = Network(args.init_channels + int(add_width[sp]), input_channels, DATASET_CLASSES1, args.layers + int(add_layers[sp]), criterion, switches_normal=switches_normal, switches_reduce=switches_reduce, p=float(drop_rate[sp]))
        model = nn.DataParallel(model)
        model = model.cuda()
        print(torch.cuda.device_count())
        
        switches_normal_save = copy.deepcopy(switches_normal)
        switches_reduce_save = copy.deepcopy(switches_reduce)
        for i in range(0,3):
            if i == 0:
                logging.info('------------------ Dataset 1: dtd')
            elif i == 1:
                logging.info('------------------ Dataset 2: birds')
                model.module.classifier = nn.Linear(128, DATASET_CLASSES2)
                model = model.cuda()
                switches_normal = copy.deepcopy(switches_normal_save)
                switches_reduce = copy.deepcopy(switches_reduce_save)
            elif i == 2:
                logging.info('------------------ Dataset 3: ImageNet')
                model.module.classifier = nn.Linear(128, DATASET_CLASSES3)
                model = model.cuda()
                switches_normal = copy.deepcopy(switches_normal_save)
                switches_reduce = copy.deepcopy(switches_reduce_save)
            ...
            for epoch in range(epochs):
                ...
                if epoch < eps_no_arch:
                    ...
                    if i == 0:
                        train_acc, train_obj = train(train_queue1, valid_queue1, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=False)
                    elif i == 1:
                        train_acc, train_obj = train(train_queue2, valid_queue2, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=False)
                    elif i == 2:
                        train_acc, train_obj = train(train_queue3, valid_queue3, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=False)
                else:
                    model.module.p = float(drop_rate[sp]) * np.exp(-(epoch - eps_no_arch) * scale_factor) 
                    model.module.update_p()   
                    if i == 0:             
                        train_acc, train_obj = train(train_queue1, valid_queue1, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=True)
                    elif i == 1:
                        train_acc, train_obj = train(train_queue2, valid_queue2, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=True)
                    elif i == 2:
                        train_acc, train_obj = train(train_queue3, valid_queue3, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=True)
                ...
                if epochs - epoch < 5:
                    if i == 0:     
                        valid_acc, valid_obj = infer(valid_queue1, model, criterion)
                    elif i == 1:
                        valid_acc, valid_obj = infer(valid_queue2, model, criterion)
                    elif i == 2:
                        valid_acc, valid_obj = infer(valid_queue3, model, criterion)
                    logging.info('Valid_acc %f', valid_acc)
            utils.save(model, os.path.join(args.save, 'weights%d.pt' % sp))
