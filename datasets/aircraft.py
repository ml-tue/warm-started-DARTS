...
def cropImage(images_dir,crop_file):
    ...
    for line in f:
        ...
        box.append([split_line[1], split_line[2], split_line[3], split_line[4]])
    f.close()
    
    for idx, img in enumerate(image_ids): 

        #print(f"DEBUG: cropImage({img}.jpg, left: {box[idx][0]}, bottom: {box[idx][1]}, right: {box[idx][2]}, top: {box[idx][3]})")

        # Open image and get height and width
        filename = os.path.join(images_dir, '%s.jpg' % (img))
        Image.MAX_IMAGE_PIXELS = 203230899200
        ...
        im.save(os.path.join(images_dir, '%s.jpg' % (img)))
...
class FGVCAircraft(data.Dataset):
    ...
    def __init__(self, root, class_type='variant', split='train', transform=None,
        ...
        self.crop_file = os.path.join(self.root, 'data', 'images_box.txt')
        self.images_dir = os.path.join(self.root, 'data', 'images')
    ...
    def _check_exists(self):
        #cropImage(self.images_dir,self.crop_file)
